...
def write_decl_header(path, size):
    guard = f"ICONS_{size}_H"
    text = f"""/**
 * Icon font for {size}px UI icons
 * Generated locally from FontAwesome 7 Free-Solid-900
 * Size: {size}px, Bpp: 4
 *
 * Generation command:
 *   scripts/bake_icons.sh
...
// Declare the icon font (defined in icons_{size}.c)
...
def write_header(path):
    lines = [
        ...
        f"// The generated {'/'.join(str(s) for s in SIZES)} px icon fonts must "
        "include these codepoints.",
    ]
...
def write_theme_fonts_header(path):
    include_blocks = "\n".join(
        f'#if !defined(ICONS_{s}) || ICONS_{s}\n#include "icons_{s}.h"\n#endif'
        for s in SIZES
    )
    case_blocks = "\n".join(
        f"#if LV_FONT_MONTSERRAT_{s} && (!defined(ICONS_{s}) || ICONS_{s})\n"
        f"  case {s}:\n"
        f"    return (theme_font_pair_t){{&lv_font_montserrat_{s}, &icons_{s}}};\n"
        f"#endif"
        for s in SIZES
    )
    fallback_blocks = "\n".join(
        f"{'#if' if i == 0 else '#elif'} "
        f"LV_FONT_MONTSERRAT_{s} && (!defined(ICONS_{s}) || ICONS_{s})\n"
        f"  return (theme_font_pair_t){{&lv_font_montserrat_{s}, &icons_{s}}};"
...
def main():
    ...
    for size in SIZES:
        ...
        write_decl_header(args.output_dir / f"icons_{size}.h", size)
    write_header(args.output_dir / "icons.h")
    write_theme_fonts_header(args.output_dir / "icons_fonts.h")
